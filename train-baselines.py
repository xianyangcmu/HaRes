...
from models.MSOPunet import MSOPUNet
#from models.eca_interp import ECA

# from models.fusion_tristream_ss import FusionMulResModel as fusion
# from models.fusion_tristream_duophase import FusionMulResModel as fusion
#from models.fusion_tristream_enhanced_up import FusionMulResModel as fusion
# from models.fusion_tristream import FusionMulResModel as fusion
# from models.fusion_tristream_nodilation import FusionMulResModel as fusion
# from models.fusion_tristream_noskip import FusionMulResModel as fusion

# from models.fusion_resunet_style import FusionMulResuModel as fusion_resu
# from models.ada_conv_unet import AdaConvModel
...
if __name__ == '__main__':
    ...
    if config["dataset"] == "LS_3CFRGB" or config["dataset"] == "LS_3CFRGB_64_7386":
    ...
    for cv_idx in range(config["num_cv"]):
        ...
        if config["selected_model"] == "fusion_interp":
            ...
            model_ft = fusion(config)
        elif config["selected_model"] == "multi3_net":
            model_ft = Multi3Net(config)
        elif config["selected_model"] == "scpf":
            model_ft = scpf()
        elif config["selected_model"] == "unet":
            model_ft = UNet(config)
        elif config["selected_model"] == "msop":
            model_ft = MSOPUNet(config)
        elif config["selected_model"] == "eca_interp":
            model_ft = ECA(config)
        elif config["selected_model"] == "multi3net_LR":
            model_ft = Multi3Net_LR(config)
#         elif config["selected_model"] == "fusion_resu":
#             model_ft = fusion_resu(config)
#         elif config["selected_model"] == "ada_conv":
#             model_ft = AdaConvModel(config)
